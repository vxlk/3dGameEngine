...
inline File getExamplesDirectory() noexcept
{
    ...
    auto exampleDir = currentFile.getParentDirectory().getChildFile ("Objects");
    ...
    while (currentFile.getFileName() != "Objects" && numTries++ < 15)
...
inline InputStream* createUIInputStream(const char* resourcePath)
{
...
#else
	auto assetsDir = getExamplesDirectory().getChildFile("UI");
#endif
...
inline Image getImageFromUI(const char* assetName)
{
	auto hashCode = (String(assetName) + "@cling_assets").hashCode64();
	auto img = ImageCache::getFromHashCode(hashCode);

	if (img.isNull())
	{
		ScopedPointer<InputStream> juceIconStream(createUIInputStream(assetName));
